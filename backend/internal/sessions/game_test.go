...
func mockLobbyGame(lobbyID string, game *Game) *mockStore {
	lobby := &Lobby{
		ID:          lobbyID,
		GameStarted: true,
		GameID:      game.ID,
	...
	return &mockStore{
		...
		getGameFn:  func(string) (*Game, error) { return game, nil },
	}
}

func askingGame(gameID, lobbyID, askerID string) *Game {
	return &Game{
		ID:      gameID,
		LobbyID: lobbyID,
		Players: []*Player{{ID: askerID, Name: "Alice"}},
		CurrentRound: &Round{
			RoundNumber: 1,
			AskerID:     askerID,
			Phase:       PhaseAsking,
...
func answeringGame(gameID, lobbyID, askerID string, existing []*Answer) *Game {
	return &Game{
		ID:      gameID,
		LobbyID: lobbyID,
		Players: []*Player{
...
func assigningGame(gameID, lobbyID, askerID string, answers []*Answer) *Game {
	return &Game{
		ID:          gameID,
		LobbyID:     lobbyID,
		TargetScore: 10,
...
func scoringGame(gameID, lobbyID, askerID string, answers []*Answer) *Game {
	return &Game{
		ID:          gameID,
		LobbyID:     lobbyID,
		TargetScore: 10,
...
func lobbyNotStarted() *mockStore {
	return &mockStore{
		getLobbyFn: func(string) (*Lobby, error) {
			return &Lobby{ID: "ABC123", GameStarted: false}, nil
		},
	}
}
...
func TestGetGame_Found(t *testing.T) {
	...
	req := withChiID(httptest.NewRequest(http.MethodGet, "/api/sessions/lobbies/ABC123/game", nil), "ABC123")
...
func TestGetGame_LobbyNotFound(t *testing.T) {
	...
	req := withChiID(httptest.NewRequest(http.MethodGet, "/api/sessions/lobbies/NOPE/game", nil), "NOPE")
	rr := httptest.NewRecorder()
	h.GetGame(rr, req)
...
func TestSubmitQuestion_HappyPath(t *testing.T) {
	game := askingGame("g1", "ABC123", "asker1")
	s := mockLobbyGame("ABC123", game)
	s.updateGameFn = func(*Game) error { return nil }
	s.broadcastToLobbyFn = noop
	h := New(s)

	body := strings.NewReader(`{"question":"What is your superpower?"}`)
	req := withChiID(httptest.NewRequest(http.MethodPost, "/api/sessions/lobbies/ABC123/question", body), "ABC123")
	...
	assert.Equal(t, PhaseAnswering, game.CurrentRound.Phase)
}
...
func TestSubmitQuestion_NoCurrentRound(t *testing.T) {
	game := &Game{ID: "g1", LobbyID: "ABC123", CurrentRound: nil}
	s := mockLobbyGame("ABC123", game)
	h := New(s)
...
func TestSubmitAnswer_HappyPath(t *testing.T) {
	...
	req := withChiID(httptest.NewRequest(http.MethodPost, "/api/sessions/lobbies/ABC123/answer", body), "ABC123")
...
func TestSubmitAnswer_ReplacesExistingAnswer(t *testing.T) {
	existing := []*Answer{{ID: "a1", PlayerID: "p2", Text: "first answer"}}
...
func TestSubmitAnswer_LastAnswerTransitionsPhase(t *testing.T) {
	...
	assert.Equal(t, http.StatusNoContent, rr.Code)
	assert.Equal(t, PhaseAssigning, game.CurrentRound.Phase)
	assert.Equal(t, 2, broadcastCount)
}

func TestSubmitAnswer_BroadcastsAnswerCount(t *testing.T) {
	game := answeringGame("g1", "ABC123", "asker1", nil)
	game.Players = append(game.Players, &Player{ID: "p3", Name: "Carol"})
	s := mockLobbyGame("ABC123", game)
	s.updateGameFn = func(*Game) error { return nil }
...
func TestAssignAnswer_HappyPath(t *testing.T) {
	answers := []*Answer{{ID: "a1", PlayerID: "p2", Text: "some answer"}}
	...
	req := withChiID(httptest.NewRequest(http.MethodPost, "/api/sessions/lobbies/ABC123/assign", body), "ABC123")
	req = withPlayerID(req, "asker1")
	rr := httptest.NewRecorder()
...
func TestAssignAnswer_PlayerNotInGame(t *testing.T) {
	answers := []*Answer{{ID: "a1", PlayerID: "p2", Text: "answer"}}
	game := assigningGame("g1", "ABC123", "asker1", answers)
	s := mockLobbyGame("ABC123", game)
	h := New(s)
...
func TestAssignAnswer_MissingFields(t *testing.T) {
	h := New(&mockStore{})
	body := strings.NewReader(`{"answerId":"a1"}`)
	req := withChiID(httptest.NewRequest(http.MethodPost, "/api/sessions/lobbies/ABC123/assign", body), "ABC123")
	req = withPlayerID(req, "asker1")
	rr := httptest.NewRecorder()
	h.AssignAnswer(rr, req)
...
func TestAssignAnswer_Reassignment(t *testing.T) {
	answers := []*Answer{{ID: "a1", PlayerID: "p2", Text: "answer", AssignedTo: "p2"}}
	game := assigningGame("g1", "ABC123", "asker1", answers)
	s := mockLobbyGame("ABC123", game)
	s.updateGameFn = func(*Game) error { return nil }
	s.broadcastToLobbyFn = noop
...
func TestLockAssignments_HappyPath(t *testing.T) {
	answers := []*Answer{
		{ID: "a1", PlayerID: "p2", Text: "answer1", AssignedTo: "p2"},
		{ID: "a2", PlayerID: "p3", Text: "answer2", AssignedTo: "p2"},
	}
	game := assigningGame("g1", "ABC123", "asker1", answers)
	s := mockLobbyGame("ABC123", game)
	s.updateGameFn = func(*Game) error { return nil }
	...
	req := withChiID(httptest.NewRequest(http.MethodPost, "/api/sessions/lobbies/ABC123/lock", nil), "ABC123")
	req = withPlayerID(req, "asker1")
	rr := httptest.NewRecorder()
	h.LockAssignments(rr, req)

	assert.Equal(t, http.StatusNoContent, rr.Code)
	assert.Equal(t, PhaseScoring, game.CurrentRound.Phase)
}

func TestLockAssignments_ScoresCorrectly(t *testing.T) {
	answers := []*Answer{
		{ID: "a1", PlayerID: "p2", Text: "ans1", AssignedTo: "p2"},
		{ID: "a2", PlayerID: "p3", Text: "ans2", AssignedTo: "p2"},
		{ID: "a3", PlayerID: "p2", Text: "ans3", AssignedTo: "p3"},
	}
	game := assigningGame("g1", "ABC123", "asker1", answers)
	s := mockLobbyGame("ABC123", game)
	s.updateGameFn = func(*Game) error { return nil }
...
func TestLockAssignments_UnassignedAnswerBlocked(t *testing.T) {
	answers := []*Answer{
		{ID: "a1", PlayerID: "p2", Text: "answer", AssignedTo: ""},
	}
	game := assigningGame("g1", "ABC123", "asker1", answers)
	s := mockLobbyGame("ABC123", game)
	h := New(s)
...
func TestNextRound_AdvancesToNextRound(t *testing.T) {
	answers := []*Answer{{ID: "a1", PlayerID: "p2", AssignedTo: "p2"}}
	...
	req := withChiID(httptest.NewRequest(http.MethodPost, "/api/sessions/lobbies/ABC123/next", nil), "ABC123")
	req = withPlayerID(req, "p2")
	rr := httptest.NewRecorder()
	h.NextRound(rr, req)

	assert.Equal(t, http.StatusNoContent, rr.Code)
	assert.Equal(t, PhaseAsking, game.CurrentRound.Phase)
...
func TestNextRound_GameOver(t *testing.T) {
	...
	assert.Equal(t, PhaseGameOver, game.CurrentRound.Phase)
}
